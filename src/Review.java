...
public Petition getNextFromWF(Petition current_petition){
	...
	if(current_petition != null){
		...
		if(!(pnw_obj.isDobInRange(current_petition.getDobYear(),current_petition.getDobMonth(),current_petition.getDobDay()))){
...
public int editMonth(Petition current_petition){
	...
	current_petition.setMonth(Month);
